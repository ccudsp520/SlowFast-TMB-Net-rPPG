...
class SlowFast_FD_Dataset(Dataset):
    def __init__(self, file, ds_name, window_length, valid=False, sliding_windows=1):
    ...
    def __getitem__(self, idx):
        ...
        if type(self.ds_name) == list:      #overlapping
            ...
            if idx+wl > (self.total_length*self.sliding_windows):   # idx overpass total_length then idx backward to wl-total_length+1
                # return None
                idx = self.total_length*self.sliding_windows - wl - 1
    ...
    def _get_arrays(self, file):
        if type(self.ds_name) == list:
            ...
            for t, f in enumerate(file):
                with tqdm(total=len(list(file[t].keys())), position=0, leave=True, desc=f'Reading from file {t}') as pbar:
                    ...
                    for i, data_path in enumerate(list(file[t].keys())):
                        ...
                        elif self.ds_name[t] == "MAHNOB_HCI":  # change here
            ...
            # self.total_length = (len(self.label)-1) // self.sliding_windows 
            self.total_length = math.ceil(len(self.label) / self.sliding_windows) - self.sw_step - 1
        else:
