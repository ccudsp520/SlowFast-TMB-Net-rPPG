...
def BPF_dict(input_val, fs):
    for index, signal in input_val.items():
        ...
        if type(fs) == list:
          low = 0.67 / (0.5 * fs[index])  # Frequency range: 0.67-2.5 Hz
          high = 2.4 / (0.5 * fs[index])
        else:
          low = 0.67 / (0.5 * fs)  # Frequency range: 0.67-2.5 Hz
          high = 2.4 / (0.5 * fs)

        # if type(fs) == list:
        #   low = 0.6 / (0.5 * fs[index])  # Frequency range: 0.67-2.5 Hz
        #   high = 2.8 / (0.5 * fs[index])
        # else:
