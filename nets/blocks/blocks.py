...
class TSM_CSTM(torch.nn.Module):
    def __init__(self, in_planes, frame_depth, shift_factor, group_on,skip):  # -> in_channels = 32 channels
        super().__init__()
        self.tsm = TSM_Block_Adv(in_planes, in_planes, frame_depth, skip, shift_factor, group_on, on=False)
        self.enhancer_1 = Local_Tempo_Enhancer(in_planes, frame_depth)  # <-TAM
