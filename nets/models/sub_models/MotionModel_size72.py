...
class MotionModel_TS_CSTM(nn.Module):
    def __init__(self, eca, in_planes, kernel_size, frame_depth, shift_factor, group_on,skip):
        super().__init__()
        self.conv_1 = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=in_planes, kernel_size=kernel_size, padding='same'),
            nn.LayerNorm([in_planes, 72, 72]),
            nn.Tanh())

        self.block_1 = TSM_CSTM(in_planes, frame_depth, shift_factor, group_on,skip)
        ...
        self.block_2 = TSM_CSTM(in_planes * 2, frame_depth, shift_factor, group_on,skip)
