...
class  LinearModel_SlowFast(torch.nn.Module):
    def __init__(self, frame_depth):
        ...
        if frame_depth%2 == 0:
            ...
            self.slow_linear = nn.Sequential(
                nn.Flatten(),
                nn.Linear(2592*(frame_depth-1), 640),
                nn.Tanh(),
                nn.Dropout(p=0.5),
                nn.Linear(640, frame_depth)
                )
        ...
        #     nn.Flatten(),
        #     nn.Linear(7776*frame_depth, 1920),
        #     nn.Tanh(),
        #     nn.Dropout(p=0.5),
        #     nn.Linear(1920, frame_depth)
        #     )
    ...
    def forward(self, fast, slow):
        ...
        #-----------------S2F-----------------#
        # ----PF_Fast PF_EF_Fast Parallel----#
        # slow_up = F.interpolate(slow.permute(0, 2, 1, 3, 4), scale_factor=(2, 1, 1), mode='trilinear').permute(0, 2, 1,
        #                                                                                                           3, 4)
        # fast = (fast + slow_up)

        #-----------------F2S-----------------#
        # ----PF_Slow PF_EF_Slow Parallel----#
        # fast_down = self.connect(fast.permute(0, 2, 1, 3, 4)).permute(0, 2, 1, 3, 4)
        # slow = (fast_down + slow)

        # print("fast:",np.shape(fast))
        # print("slow:",np.shape(slow))

        # cat_feature = torch.cat((fast,slow), dim=1)
        # print("cat_feature:",np.shape(cat_feature))

        # out = self.fast_linear.forward(cat_feature)
        # out = self.cat_linear.forward(cat_feature)
